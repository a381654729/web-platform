$(function () {
  ...
  $("#chart").parent().parent().addClass('active');
  ...
  loadGauge();
  loadStep();
  $("#chart").html('<a href="javascritp:void(0);"><i class="fa fa-pie-chart"></i> 基本图形</a>');
});

function loadBar(myChart) {
    ...
    success: function (rdata) {
      if (rdata) {
        for (var i = 0; i < rdata.length; i++) {
          names.push(rdata[i].username);
          scores.push(rdata[i].score)
        }
        myChart.hideLoading();
        myChart.setOption({
          title: {
